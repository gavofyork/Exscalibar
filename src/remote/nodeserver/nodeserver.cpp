...
public:
	NodeServerSession(int socket, NodeServer *server, SessionServer *session): theSource(new QTcpSocket), theSession(session), theServer(server)
	{
		theSource.sd()->setSocketDescriptor(socket);
...
public:
	NodeServer(uint port = RGEDDEI_PORT) : QTcpServer() { listen(QHostAddress::LocalHost, port); }
};
...
void NodeServerSession::run()
{
	...
	while (theSource.isOpen())
	{
		...
		case NewProcessor:
		{
			...
			if (MESSAGES) qDebug("Creating Processor %s of type %s", qPrintable(name), qPrintable(type));
			bool ret;
		...
		{
			QString name = theSource.receiveString();
			if (MESSAGES) qDebug("Deleting Processor %s", qPrintable(name));
		...
		case NewDomProcessor:
		{
			QString type = theSource.receiveString();
			QString name = theSource.receiveString();
			if (MESSAGES) qDebug("Creating DomProcessor %s of SubProcessor type %s", qPrintable(name), qPrintable(type));
			bool ret;
		...
		{
			QString name = theSource.receiveString();
			if (MESSAGES) qDebug("Destroying DomProcessor %s", qPrintable(name));
		...
		{
			QString name = theSource.receiveString();
			if (MESSAGES) qDebug("Starting Processor %s", qPrintable(name));
			bool ret;
		...
		{
			QString name = theSource.receiveString();
			if (MESSAGES) qDebug("Local Connecting Processor %s", qPrintable(name));
			...
			if (MESSAGES) qDebug("Connecting from %s:%d to %s:%d (size:%d)", qPrintable(name), output, qPrintable(destName), destInput, bufferSize);
			bool ret;
