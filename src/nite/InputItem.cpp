...
InputItem::InputItem(int _i, ProcessorItem* _p, QSizeF const& _size): QGraphicsItem(_p), m_index(_i), m_baseSize(_size)
{
	m_size = m_baseSize;
}
...
void InputItem::paint(QPainter* _p, const QStyleOptionGraphicsItem*, QWidget*)
{
	...
	if (processorItem()->m_processor->isRunning())	//  a bit unsafe, since the processor could stop & get reset between these two.
	{
		...
		_p->fillRect(QRectF(0, -m_size.height() / 4, -(m_size.width() - m_baseSize.width()) * fill, m_size.height() / 4) , QColor(Qt::darkRed));
	}
}
