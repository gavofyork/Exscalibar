...
bool ALSAPlayer::processorStarted()
{
	...
	else if (snd_pcm_hw_params_set_rate_resample(thePcmHandle, hwparams, (uint)input(0).readType().asA<Contiguous>().frequency()))
		fprintf(stderr, "The rate %d Hz is not supported by your hardware.\n", (uint)input(0).readType().asA<Contiguous>().frequency());
